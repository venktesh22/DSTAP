 ...
 * @author vp6258
 */
public class ODLinksSpectralAlgo extends PartitioningAlgo{
    private DoubleMatrix weightedAdjacencyMatrix;
    private DoubleMatrix diagonalDegreeMatrix;
    
    public ODLinksSpectralAlgo(int noOfClusters, String netName) {
...
////        prettyPrintMatrix(secondSmallestEigenVector);
        ...
        //find maxODdemand
        double maxODDemand = Double.MIN_VALUE;
       for(Integer originID: this.network.odDemand.keySet()){
           for(Integer destID: this.network.odDemand.get(originID).keySet()){
               double demand=this.network.odDemand.get(originID).get(destID);
               if(demand>maxODDemand){
                   maxODDemand = demand;
               }
           }
       }
       ...
       //Assumming minNOdeId=0 for Anaheim network
       if(network.networkName.equals("Anaheim"))
            minNodeID = 0;
       ...
       for(Link l: network.getLinks()){
           ...
           weightedAdjacencyMatrix.put(sourceNodeID-minNodeID, destNodeID-minNodeID, this.network.UELinkFlow.get(l)+maxODDemand); //LINKFLOW);
...
//                   diagonalDegreeMatrix.get(destNodeID-minNodeID, destNodeID-minNodeID)+1);
       ...
       //add negative of demand
       for(Integer originID: this.network.odDemand.keySet()){
           for(Integer destID: this.network.odDemand.get(originID).keySet()){
               double demand=this.network.odDemand.get(originID).get(destID);
               weightedAdjacencyMatrix.put(originID, destID, 
                       weightedAdjacencyMatrix.get(originID,destID)-1*demand+maxODDemand);
           }
       }
       
       
        //matrix is now signed
        ...
        System.out.println("====Symmetric adjacency matrix====");
        prettyPrintMatrix(symAdjMat);
        System.out.println("\n\n");
        
        for(int i=0;i< symAdjMat.rows;i++){
           double sumRow=0.0;
           for(int j=0;j<symAdjMat.columns;j++)
               sumRow+= Math.abs(symAdjMat.get(i,j));
           diagonalDegreeMatrix.put(i, i,(sumRow)); //ensuring that the diagonal matrix is positive definite so that its square root can be found
       }
        
        System.out.println("====Diagonal degree matrix====");
        prettyPrintMatrix(diagonalDegreeMatrix);
        System.out.println("\n\n");
       //generate Laplacian matrix and normalize it
...
//       
       ...
       DoubleMatrix secondSmallestEigenVector =  eigenValueAndVectors[0].getColumn(1); //first eigenvalue
        System.out.println("Eigenvector corresponding to second smallest eigenvalue is");
        prettyPrintMatrix(secondSmallestEigenVector);
        
        System.out.println("Eigence corresponding to smallest eigenvalue of zero:");
        prettyPrintMatrix(eigenValueAndVectors[0]);
...
//       //--------------------Identify boundary nodes--------------------
        findBoundaryNodesAndRegionalLinks();
...
//    }
    ...
    @Override
    protected void setOutputFolderName(String s){
        String epoch= Integer.toString((int)(System.currentTimeMillis()/1000.0));
        epoch = "ODSpectral"+"_"+s+"_"+epoch;
        File dir = new File("Networks/"+netName+"/Inputs/"+epoch);

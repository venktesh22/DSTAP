...
Tasks:
1) Automate sub-level partitions
    ...
    def populate_from_bargera(self, textfile):
        ...
        try:
            with open(textfile) as bargera:  # use with to avoid having to close the file
                first_line = bargera.readline()
                delimited = first_line.split()
                num_zones = delimited[-1]
                ...
                while True:
                    try:
                        ...
                        self.add_edge(int(lod[0]), int(lod[1]),
                                      {'capacity': float(lod[2]), 'length': float(lod[3]), 'FFT': float(lod[4]),
                                       'b': float(lod[5]), 'Power': float(lod[6]),
            ...
            with open(textfile) as results:
                ...
                while True:
                    try:
                        ...
                        # lod=newdata.split(' ')  #list of data, zero is first node, 1 is second node, 2 is volume,
                        # 3 is cost
                        ...
                        newdata = data.rstrip() # This might not even be necessary if you just split()
                        lod = newdata.split('\t')  # If you don't specify it automatically checks \t or ' ', can fix
...
MyGraph = networkcny()
MyGraph.populate_from_bargera('TexasSAM_net.txt')
MyGraph.populate_volumes_costs('TexasSAM_flow.txt')

number_of_zones = float(MyGraph.zones)  # Gets the number of zoens in graph, needed to skip centroids
...
for i in mylist:
    for j in mylist[mylist.index(i) + 1:]:
        # There are links in the assignment file that are removed since
        # they are not present in the network file, or vice verse
        # Those are artificial links that are created in the bush based analysis
        # print(MyGraph[i][j])
        ...
        if MyGraph.has_edge(i, j):
            if MyGraph.has_edge(j, i):
                tempv1 = MyGraph[i][j]['volume']  # can use deepcopy instead
...
for edge in UndirectedGraph.edges():
    if UndirectedGraph[edge[0]][edge[1]]['volume'] == 0:
        print('removed edge' + str(edge[0]) + str(edge[1]))
...
# You are getting the connected component after deleting the links
# Those components will be separate nodes, each one node.
# Create a copy graph and then just add them to partition where
# the adjacent nodes belong
# Note that if you remove a link, the nodes on it's ends may
# still be part of the connected component if they are not isolated
# by the link removal, not every link you remove you will have to
# add its nodes
...
# Those are given high indexes, 10001, 10002, 1003, to indicate they are separated
# because they belong to different components
# 
# NEED TO FIX THIS
#

# Note that in most cases the number of connected components with +ve flow is one!
...
for NewUG in graphs:
    ...
    sub_right = NewUG.subgraph(rightHalf)
    secondEigenvectorsub_right = linalg.fiedler_vector(sub_right, weight='volume', normalized='True')
    listofnodessub_right = sub_right.nodes()
    orderednodessub_right = [x for _,x in sorted(zip(secondEigenvectorsub_right, listofnodessub_right))]
    countsub_right=0
    for elem in secondEigenvectorsub_right:
        if elem < 0:
            countsub_right=countsub_right+1

    leftHalfsub_right = list(sorted(orderednodessub_right[:countsub_right]))
    rightHalfsub_right = list(sorted(orderednodessub_right[countsub_right:]))

    partitionsub_right = leftHalfsub_right, rightHalfsub_right
    # writetofile(partitionsub_right, 'AustinFlowWeightedFourRightHalf')  # add missing nodes before writing to file
    ...
    sub_left = NewUG.subgraph(leftHalf)
    secondEigenvectorsub_left = linalg.fiedler_vector(sub_left, weight='volume', normalized='True')
    listofnodessub_left = sub_left.nodes()
    orderednodessub_left = [x for _, x in sorted(zip(secondEigenvectorsub_left, listofnodessub_left))]
    countsub_left = 0
    for elem in secondEigenvectorsub_left:
        if elem < 0:
            countsub_left = countsub_left + 1

    leftHalfsub_left = list(sorted(orderednodessub_left[:countsub_left]))
    rightHalfsub_left = list(sorted(orderednodessub_left[countsub_left:]))

    partitionsub_left = leftHalfsub_left, rightHalfsub_left
    # writetofile(partitionsub_left, 'AustinFlowWeightedFourRightHalf')    # add missing nodes before writing to file

    # *****************************************************
    """

    # Check if the nodes to be added are in left partition or right partition
    # Note there's a bug in the script below, it only works if there's one connected component with +ve flow
    # The bug is in the missing nodes part, it has to check the missing nodes relevant to each component
    '''
    copied_nodes = copygraph.nodes()
    partitioned_nodes = NewUG.nodes()
    missing_nodes = list(set(copied_nodes) - set(partitioned_nodes))  # the nodes were removed as components
    for node in missing_nodes:
        # assert isinstance(copygraph, object)
        neighbors = copygraph.neighbors(node)
        set_neighbors = set(neighbors)
        set_left = set(leftHalf)  # The sets need to be updated at each iteration
        set_right = set(rightHalf)  # for nodes that don't make it into left or right first round (neighbors not there)
        left_intersect = set_neighbors.intersection(set_left)
        right_intersect = set_neighbors.intersection(set_right)

        if bool(left_intersect):
            leftHalf.append(node)
        elif bool(right_intersect):
            rightHalf.append(node)
        else:
            missing_nodes.append(node)
        # missing_nodes.append(node)  # add the node to end of queue in hope of adding a neighbor that has neighbors
        # in the sets leftHalf or rightHalf before you reach it. Example if two nodes are removed where one is
        # connected to network and another is connected only to that node
    '''
    """
    #INDENT THIS
    set_left_left = set(leftHalfsub_left)
    set_right_left = set(rightHalfsub_left)  # The right half subpartition of the leftHalf
    set_left_right = set(leftHalfsub_right)
    set_right_right = set(rightHalfsub_right)

    left_left_intersect = set_neighbors.intersection(set_left_left)
    right_left_intersect = set_neighbors.intersection(set_right_left)
    left_right_intersect = set_neighbors.intersection(set_left_right)
    right_right_intersect = set_neighbors.intersection(set_right_right)

    if bool(left_left_intersect):
        leftHalfsub_left.append(node)
    elif bool(right_left_intersect):
        rightHalfsub_left.append(node)
    elif bool(set_left_right):
        leftHalfsub_right.append(node)
    elif bool(set_right_right):
        rightHalfsub_right.append(node)
    else:
        missing_nodes.append(node)
    # missing_nodes.append(node)  # add the node to end of queue in hope of adding a neighbor that has neighbors
    # in the sets leftHalf or rightHalf before you reach it. Example if two nodes are removed where one is
    ...
    # partition = leftHalfsub_left, rightHalfsub_left, leftHalfsub_right, rightHalfsub_right
    # Write the partition to file
    # name = 'partition'
    # Create text file
    # writetofile(partition, countpartition, name)
    # update the partition iterator
    # countpartition += 2  # You know that you will partition each component into two subcomponents!


    # Define the partition
    partition_original = leftHalf, rightHalf
    # Write the partition to file
    name = 'partitionTX' + str(countpartition) + str(countpartition + 1)
    # Create text file
    writetofile(partition_original, countpartition, name)
    # update the partition iterator
    countpartition += 2  # You know that you will partition each component into two subcomponents!
